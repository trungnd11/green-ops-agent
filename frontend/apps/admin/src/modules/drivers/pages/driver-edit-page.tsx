...
import { fetchDriver, updateDriver } from "../api/driver.api";
...
export function DriverEditPage() {
  const { driverId } = useParams({ from: "/_authenticated/drivers/$driverId/edit" });
  ...
  const vals = useRef({
    driverCode: "", fullName: "", phone: "", email: "", cccd: "",
    birthDate: "", gender: "male", address: "", licenseNumber: "", licenseClass: "",
    joinDate: "", status: "active", depositAmount: "", note: "", referrerId: "",
  });
  ...
  useEffect(() => {
    if (driver) {
      vals.current = {
        ...
        email: driver.email || "",
        cccd: driver.cccd || "",
        birthDate: fmt(driver.birthDate),
        gender: driver.gender || "male",
        address: driver.address || "",
        licenseNumber: driver.licenseNumber || "",
        licenseClass: driver.licenseClass || "",
        joinDate: fmt(driver.joinDate),
        status: driver.status || "active",
        depositAmount: driver.depositAmount ? String(driver.depositAmount) : "",
        note: driver.note || "",
  ...
  const validate = () => {
    ...
    if (!vals.current.driverCode.trim()) errs.driverCode = "Vui lòng nhập mã LX";
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        driverCode: vals.current.driverCode.trim(),
        fullName: vals.current.fullName.trim(),
        phone: vals.current.phone.trim() || undefined,
        email: vals.current.email.trim() || undefined,
        cccd: vals.current.cccd.trim() || undefined,
        gender: vals.current.gender || undefined,
        address: vals.current.address.trim() || undefined,
        licenseNumber: vals.current.licenseNumber.trim() || undefined,
        licenseClass: vals.current.licenseClass.trim() || undefined,
        status: vals.current.status,
        depositAmount: vals.current.depositAmount ? Number(vals.current.depositAmount.replace(/[^0-9]/g, "")) : undefined,
        note: vals.current.note.trim() || undefined,
      };
      if (vals.current.birthDate) payload.birthDate = vals.current.birthDate;
      if (vals.current.joinDate) payload.joinDate = vals.current.joinDate;
      if (vals.current.referrerId) payload.referrerId = vals.current.referrerId;

      await updateDriver(driverId, payload);
      queryClient.invalidateQueries({ queryKey: ["drivers"] });
      queryClient.invalidateQueries({ queryKey: ["driver", driverId] });
      notification.success({ message: "Cập nhật tài xế thành công", placement: "bottomRight" });
      navigate({ to: `/drivers/${driverId}` } as any);
    } catch (err: any) {
  ...
  if (isLoading) {
    return (
      <div className="flex animate-pulse flex-col gap-4 max-w-160 mx-auto">
  ...
  if (!driver) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20">
        <span className="text-[16px] font-semibold text-text-primary">Không thể tải thông tin tài xế</span>
    ...
    <div className="relative mx-auto max-w-160">
      <div className="flex items-center justify-between mb-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[13px] text-text-tertiary">Cập nhật thông tin tài xế.</p>
        </div>
        <button
          className="flex items-center justify-center w-9 h-9 rounded-btn bg-bg-subtle"
          onClick={() => navigate({ to: `/drivers/${driverId}` } as any)}
        >
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
        ...
        >
          ...
          >
            <Button variant="secondary" onClick={() => navigate({ to: `/drivers/${driverId}` } as any)}>
