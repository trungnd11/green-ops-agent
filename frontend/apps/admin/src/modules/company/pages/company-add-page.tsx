...
import { ArrowLeft, ChevronRight, Building2, Plus, X } from "lucide-react";
import { notification } from "@xanh/ui/notification";
import { createCompany } from "../../admin/api/user.api";

export function CompanyAddPage() {
  const navigate = useNavigate();

  const [form, setForm] = useState({
    code: "", name: "", address: "", phone: "", email: "", taxCode: "", contactPerson: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notification.success({ message: "Thêm công ty thành công", placement: "bottomRight" });
      navigate({ to: "/admin/companies" } as any);
    } catch (err: any) {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-8 h-16 border-b shrink-0" style={{ borderColor: "rgba(255,255,255,0.12)" }}>
        ...
        <span className="text-[13px] text-text-tertiary flex items-center gap-1.5">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="text-text-primary">Thêm công ty</span>
        </span>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold text-text-primary" style={{ fontFamily: "Manrope" }}>Thêm công ty mới</h1>
              <p className="text-[13px] text-text-tertiary">Nhập thông tin công ty mới</p>
            </div>
          </div>
          ...
          <div className="rounded-card border p-6" style={{ borderColor: "rgba(255,255,255,0.12)", background: "#1C2737" }}>
            ...
            <div className="flex items-center gap-3 pt-5">
              <button className="rounded-btn border px-5 h-11 text-[14px] font-medium flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.12)", color: "#F8FAFC", background: "transparent" }} onClick={() => navigate({ to: "/admin/companies" } as any)}>
